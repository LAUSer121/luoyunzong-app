...
- 🎵 音乐：本地曲目 + 网易云「在线修仙电台」（带封面、可拖动进度、启动自动播放开关）
- 🈶 字体用系统自带：Windows 微软雅黑、macOS 苹方/系统默认，避免回退到日文字形
- 🖼 修仙风应用图标：仙山 + 灵珠 + 鎏金圆环（`tool/make_icon.dart` 程序化生成）
- 💾 数据层抽象：默认本地存储（单文件存档），预留 HTTP + MySQL 接口实现，切换只需改一行配置
- 🔁 兼容旧存档：可直接导入旧版 `落云宗存档.js` / `.json`
- ⚙️ GitHub Actions：多端并行构建，每次构建自动发布单文件 Release
...
**每次推送到 `main` 都会自动发布一个滚动 Release（tag `latest`）**，六个单文件产物直接挂在那里，
不打包、不压缩：打开仓库的 Releases 页面即可逐个下载。

## 一·五、音乐与在线修仙电台

右下角音乐悬浮球提供：封面、可拖动进度条（本地与在线曲目都支持）、上一首/下一首、音量、
曲单管理、**在线修仙电台**。

- **在线修仙电台**：内置「仙侠 / 古风 / 仙气纯音乐 / 洞箫古筝 / 御剑江湖 / 宗门修炼 / 山河仙途 / 打坐冥想」
  等关键词，搜索网易云歌曲，列表带封面，可「试听」或「加入曲单」（加入后写入存档，下次自动带上）。
- **一键仙侠电台**：音乐面板上的「一键仙侠电台」按钮（电台弹窗里是「一键挑选」）会随机换关键词搜索、
  逐个校验可播放性，自动挑出 6 首能播的仙侠曲目并开始播放；**启动时若曲单为空且开启自动播放，
  也会自动挑一批并播放**，不需要手动搜索。
- **启动自动播放开关**：音乐面板与「设置 → 音乐与在线电台」里都能切换，关闭后进入应用不自动播放。
- 桌面 / 手机版直连网易云官方接口；浏览器版受跨域限制，可在「设置 → 在线电台代理」填写自建
  [NeteaseCloudMusicApi](https://github.com/Binaryify/NeteaseCloudMusicApi) 地址（如 `http://127.0.0.1:3000`）。
  在线曲目只保存歌名/歌手/封面，播放时按 id 取流地址，不下载、不缓存受版权保护的内容。
- **播放兼容**：网易云外链是 302 跳转，Windows 的 Media Foundation 不认这种无扩展名地址
  （会报 `WindowsAudioError: Failed to set source`）。客户端会先手动跟随跳转拿到 CDN 直链
  （`audio/mpeg`）再交给播放器；若仍失败，则回退为「下载到内存后播放」。
- 封面来自网易云 CDN；若加载失败会显示音符占位图。
